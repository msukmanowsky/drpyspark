...
with pyspark.SparkContext() as sc:
    DRACULA_CH_1 = DRACULA_CH_1.translate(None, string.punctuation)
    lines = [l.strip() for l in DRACULA_CH_1.split('\n') if l.strip()]
    lines = sc.parallelize([DRACULA_CH_1])
    words = (lines.flatMap(lambda l: l.split())
             .filter(lambda word: word.strip() != '' and not word.isdigit()))
    word_count = (words.map(lambda word: (word, 1))
                  .reduceByKey(add))

    word_count = sorted(word_count.collectAsMap().items(),
                        key=lambda (w, c): c, reverse=True)
    pprint.pprint(word_count[:10])
